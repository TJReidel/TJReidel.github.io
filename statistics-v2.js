...
(function (global) {
  ...
  function intakeKey(date, medicationId, scheduledTime) {
    ...
    for (var m = 0; m < meds.length; m++) {
      ...
      for (var d = 0; d < ds.length; d++) {
        ...
        for (var t = 0; t < times.length; t++) {
          ...
          out.documented++;
          if (n.tier === api().TIER.GREEN) out.green++;
          else if (n.tier === api().TIER.YELLOW) out.yellow++;
          else if (n.tier === api().TIER.RED) out.red++;
          else out.unrated++;
        }
      }
    }
  ...
  function motivation(stats, lang) {
    var de = lang === "de";
    if (!stats || stats.total === 0 || stats.documented === 0) {
      return de ? "Jeder Eintrag hilft Ihnen, den Überblick zu behalten." : "Every entry helps you keep an overview.";
    }
    if (stats.documented === stats.total) {
      return de ? "Alles dokumentiert – gut im Blick." : "Everything documented – well in view.";
    }
  ...
  function swatch(kind) {
    var styles = {
      green:"background:var(--teal);color:#fff",
      yellow:"background:#d7b46a;color:var(--ink)",
      red:"background:#c94a3b;color:#fff",
      unrated:"background:#ece9e4;color:#2f2c29;border:1px solid #8a847d",
      undocumented:"background:#fff;color:var(--ink2);border:1px solid var(--cream2)"
    };
  ...
  function enhance() {
    ...
    var titleText = typeof global.tr === "function" ? global.tr("statsTitle") : null;
    var sections = document.querySelectorAll(".settings-section");
    var section = null;
    for (var i = 0; i < sections.length; i++) {
      var title = sections[i].querySelector(".settings-title");
      if (title && (!titleText || title.textContent.trim() === titleText)) { section = sections[i]; break; }
    }
    if (!section || section.dataset.statsV2 === "1") return;
    var card = section.querySelector(".settings-card");
    if (!card) return;

    var stats = breakdown(global.S.taken, global.S.meds, global.getPastDays(global.S.period));
    var labels = LABELS[global.S.lang] || LABELS.en;
    var rows = [
      ["green",labels.green,stats.green],
      ["yellow",labels.yellow,stats.yellow],
      ["red",labels.red,stats.red],
      ["unrated",labels.unrated,stats.unrated],
      ["undocumented",labels.undocumented,stats.undocumented]
    ];

    var block = document.createElement("div");
    block.setAttribute("data-statistics-v2", "1");
    ...
    for (var r = 0; r < rows.length; r++) {
      block.innerHTML += '<div class="settings-row" style="min-height:50px"><div style="display:flex;align-items:center;gap:10px">' + swatch(rows[r][0]) + '<div class="settings-row-label" style="font-size:14px">' + rows[r][1] + '</div></div><div style="font-size:18px;font-weight:800;color:var(--ink2)">' + rows[r][2] + '</div></div>';
    }

    var firstRow = card.querySelector(".settings-row");
    if (firstRow && firstRow.nextSibling) card.insertBefore(block, firstRow.nextSibling);
    else card.appendChild(block);
    section.dataset.statsV2 = "1";
  }
  ...
  if (typeof document !== "undefined") {
    document.addEventListener("DOMContentLoaded", function () {
      ...
      if (app && typeof MutationObserver !== "undefined") {
        new MutationObserver(function(){ enhance(); }).observe(app,{childList:true,subtree:true});
      }
    }, {once:true});
  }
